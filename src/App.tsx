...
const [aiSettings, setAiSettings] = useState<AISettings>({ 
  ...
  // --- 這裡是你原本的萬用連接器 (保持不變) ---
  const callUniversalAI = async (history: Message[], systemPrompt: string, retryCount = 0): Promise<string> => {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          // 建議這裡也順便限制歷史紀錄數量，減少 Token 消耗
          ...history.slice(-15).map(m => ({ // 只取最近 15 條
    ...
    if (response.status === 429) {
      ...
      if (retryCount < maxRetries) {
        // 計算等待時間：第一次 2秒, 第二次 4秒, 第三次 8秒 (指數成長)
        const waitTime = Math.pow(2, retryCount) * 2000; 
        console.warn(`觸發 API 限制 (429)，${waitTime/1000} 秒後進行第 ${retryCount + 1} 次重試...`);
        
        await new Promise(resolve => setTimeout(resolve, waitTime));
        return callUniversalAI(history, systemPrompt, retryCount + 1); // 遞迴呼叫
      }
    }
...
const handleSendMessage = async (text?: string | string[]) => {
  ...
  const handleCharacterChat = async (character: Character, text: string | string[]) => {
    ...
    const trimmedHistory = character.messages.slice(-10); 
    const currentHistoryForAI = [...trimmedHistory, ...userMsgs];
...
要求：1. 遵守性格。2. 暱稱是「${character.userNickname || userProfile.name}」。3. 多用短句。`;
      ...
      // --- 改用通用呼叫 ---
      const responseText = await callUniversalAI(currentHistoryForAI, systemPrompt);
